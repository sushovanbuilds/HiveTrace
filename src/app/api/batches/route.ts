...
import { generatePublicCode, issueQRToken, qrVerificationUrl } from "@/lib/services/qr";
...
const CreateSchema = z.object({
  ...
  organisationId: z.string().trim().min(1).max(64).optional(),
  /** QR customization options — when provided, a QR code is auto-generated with the batch. */
  qrPreset: z.string().optional(),
  qrDark: z.string().optional(),
  qrLight: z.string().optional(),
  qrLogo: z.string().optional(),
  qrModuleRadius: z.number().min(0).max(0.5).optional(),
});
...
export const POST = route(async (request: NextRequest) => {
  ...
  // Auto-generate a QR token and SVG for the new batch.
  const token = issueQRToken(batch.id, batch.publicCode);
  const qrUrl = qrVerificationUrl(batch.publicCode, token);

  const qrOptions: QrSvgOptions = {
    title: `Verification QR for batch ${batch.publicCode}`,
  };
  if (body.data.qrPreset) qrOptions.preset = body.data.qrPreset;
  if (body.data.qrDark) qrOptions.dark = body.data.qrDark;
  if (body.data.qrLight) qrOptions.light = body.data.qrLight;
  if (body.data.qrLogo) qrOptions.logo = body.data.qrLogo;
  if (body.data.qrModuleRadius !== undefined) qrOptions.moduleRadius = body.data.qrModuleRadius;

  const svg = qrToSvg(qrUrl, qrOptions);

  const qrRecord = await db.qRToken.create({
    data: { batchId: batch.id, token, expiresAt: null },
  });
  ...
  await db.batchEvent.create({
    data: {
      batchId: batch.id,
      type: "QR_ISSUED",
      data: { qrTokenId: qrRecord.id, autoGenerated: true },
      actorId: auth.user.id,
    },
  });
